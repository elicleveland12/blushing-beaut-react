...
export default class DesktopCart extends Component {
  ...
  handleToken = async(token) => {
    const product = await
      {
        ...
        fontStyle: `font style: ${this.state.currentCart.map(item => {return item.fontStyle})} - `,
        paintType: `paint style: ${this.state.currentCart.map(item => {return item.paintType})} - `,
        size: `cup size: ${this.state.currentCart.map(item => {return item.size})} - `,
        swirlColors: `swirl colors ${this.state.currentCart.map(item => {return item.swirlColors})} - `,
        decal: `decal ? ${this.state.currentCart.map(item => {return item.decal})} - `,
        phoneNum: `phoneNum: ${this.state.currentCart.map(item => {return item.phoneNum})} - `,
    ...
    if (Object.keys(product).length > 0) {
      const response = await axios.post("https://blushing-beaut-node.herokuapp.com/checkout", {token, product})
